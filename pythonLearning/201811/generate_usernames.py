...
def generate_username(fields, usernames):
    ...
    username = originnal_name = username[:8].lower()
    count = 1
    while username in usernames:
        username = f'{originnal_name}{count}'
        count += 1
...
def print_users(users):
    ...
    print("{0:-<{nw}}{1:-<6}{0:<{uw}}".format("Name", "ID", "Username", nw=namewidth, uw=usernamewidth))
    for key in sorted(users):
        user = users[key]
        initial = ""
        if user.middlename:
            initial = " " + user.middlename[0]
        name = f"{user.surname}, {user.forname}{initial}"
